...
const createRegistration = async (req, res) => {
  ...
  if (!firstName || !parentPhone)
    throw new AppError('First name and parent phone required', 400);
  ...
  res.status(201).json({ success: true, data: row, message: 'Registration submitted' });
};
...
const updateRegistration = async (req, res) => {
  ...
  res.json({ success: true, data: row, message: 'Registration updated' });
};
...
const convertToStudent = async (req, res) => {
  ...
  // Create student — sequential queries, NO transaction
  ...
  res.status(201).json({
    ...
    message: `Admitted successfully! No: ${admNo}`
  });
};
