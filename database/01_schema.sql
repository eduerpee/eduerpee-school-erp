...
-- FILE: 01_schema.sql  |  Run this FIRST
-- Creates all 45+ tables across 18 modules
...
-- ============================================================
DO $$ BEGIN CREATE TYPE user_role        AS ENUM ('super_admin','school_admin','principal','teacher','accountant','receptionist','parent','student'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE category_type   AS ENUM ('general','obc','sc','st','ews'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE attendance_status AS ENUM ('present','absent','leave','late','holiday'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE fee_payment_mode AS ENUM ('cash','upi','bank_transfer','cheque','online'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE fee_status       AS ENUM ('paid','partial','pending','waived'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE enquiry_status   AS ENUM ('new','follow_up','interested','not_interested','converted'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE employee_type    AS ENUM ('teacher','accountant','receptionist','driver','peon','principal','admin','librarian','security'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE exam_type        AS ENUM ('unit_test','half_yearly','annual','quarterly','practical'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE notice_type      AS ENUM ('general','circular','event','holiday','urgent'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE book_status      AS ENUM ('available','issued','reserved','damaged','lost'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ============================================================
-- MODULE 1: MULTI-SCHOOL MANAGEMENT
-- ============================================================
...
-- ============================================================
-- MODULE 3: SCHOOL SETUP
-- ============================================================
CREATE TABLE IF NOT EXISTS classes (
    id               UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    school_id        UUID REFERENCES schools(id) ON DELETE CASCADE,
    academic_year_id UUID REFERENCES academic_years(id),
    name             VARCHAR(50)  NOT NULL,    -- 'Class 6', 'Class 7' …
    numeric_level    INTEGER,                  -- 6, 7, 8 … for sorting
    description      TEXT,
    is_active        BOOLEAN DEFAULT TRUE,
    created_at       TIMESTAMPTZ DEFAULT NOW(),
...
-- ============================================================
-- MODULE 10: EXAMINATIONS & RESULTS
-- ============================================================
...
CREATE TABLE IF NOT EXISTS vehicles (
    ...
    school_id        UUID REFERENCES schools(id),
    registration_no  VARCHAR(30) UNIQUE NOT NULL,
    vehicle_type     VARCHAR(20) DEFAULT 'school_bus',
...
CREATE INDEX IF NOT EXISTS idx_students_class         ON students(current_class_id, current_section_id);
CREATE INDEX IF NOT EXISTS idx_students_adm_no        ON students(admission_no);
CREATE INDEX IF NOT EXISTS idx_students_status        ON students(school_id, status);
CREATE INDEX IF NOT EXISTS idx_att_date               ON student_attendance(date, school_id);
CREATE INDEX IF NOT EXISTS idx_att_student            ON student_attendance(student_id, date);
CREATE INDEX IF NOT EXISTS idx_fee_pay_student        ON fee_payments(student_id);
CREATE INDEX IF NOT EXISTS idx_fee_pay_date           ON fee_payments(payment_date);
CREATE INDEX IF NOT EXISTS idx_fee_pay_school         ON fee_payments(school_id, is_cancelled);
CREATE INDEX IF NOT EXISTS idx_student_fees_student   ON student_fees(student_id, status);
CREATE INDEX IF NOT EXISTS idx_employees_school       ON employees(school_id);
CREATE INDEX IF NOT EXISTS idx_exam_marks_student     ON student_marks(student_id, exam_id);
CREATE INDEX IF NOT EXISTS idx_notices_school         ON notices(school_id, is_published);
CREATE INDEX IF NOT EXISTS idx_audit_user             ON audit_logs(user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_users_school           ON users(school_id, role);
CREATE INDEX IF NOT EXISTS idx_enquiries_status       ON enquiries(school_id, status);
...
CREATE INDEX IF NOT EXISTS idx_books_school           ON books(school_id, status);
CREATE INDEX IF NOT EXISTS idx_payroll_emp            ON payroll(employee_id, month_year);
...
CREATE TRIGGER trg_enquiries_updated BEFORE UPDATE ON enquiries FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
INSERT INTO role_permissions (role, module, action) VALUES
  ...
  ('principal','attendance','read'),  ('principal','fees','read'),
  ('principal','exams','read'),       ('principal','employees','read'),
  ('principal','notices','create'),   ('principal','reports','read'),
  ...
  ('teacher','students','read'),      ('teacher','timetable','read'),

  -- Accountant
  ('accountant','fees','*'),          ('accountant','expenses','*'),
  ('accountant','payroll','*'),       ('accountant','reports','read'),
...
BEGIN
  ...
  RAISE NOTICE '✅ Schema ready! % tables created.', tbl_count;
END $$;
